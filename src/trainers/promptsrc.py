    ...
    def __init__(self, cfg, classnames, clip_model):
        ...
        with torch.no_grad():
            ...
            for single_template in IMAGENET_TEMPLATES:
                ...
                # because of memory constraints, we need to split the text features
                # if you have enough memory, you can use the commented line below to get all text features at once
                # text_features = clip_model_temp.encode_text(x_tokenized.cuda())
                text_features = torch.cat(
                    [clip_model_temp.encode_text(batch_prompts.cuda()) for batch_prompts in
                     torch.split(x_tokenized, 8000)]
                )
